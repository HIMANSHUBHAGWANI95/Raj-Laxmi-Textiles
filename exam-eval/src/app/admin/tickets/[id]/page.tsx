...
export default function AdminTicketDetailPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status.toUpperCase()) {
      ...
      case "CLOSED":
        return "bg-paper text-ink border-rule";
      default:
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-paper dark:bg-black flex items-center justify-center p-6">
        <div className="text-center">
          <Loader2 className="w-10 h-10 text-blue-500 animate-spin mx-auto mb-4" />
          <p className="text-graphite text-sm font-medium">Loading support ticket details...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-paper dark:bg-black p-4 sm:p-6 lg:p-8">
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-6">
        
        {/* Left Column: Chat replies or internal private notes */}
        <div className="flex-1 flex flex-col bg-surface rounded-2xl border border-rule shadow-sm overflow-hidden min-h-[500px]">
          
          {/* Header Area */}
          <div className="border-b border-rule p-4 bg-gray-50/50 flex flex-wrap items-center justify-between gap-4 select-none">
              ...
              <button
                ...
                className={`text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${
                  activePane === "chat"
                    ...
                    : "text-graphite hover:text-ink dark:hover:text-gray-100"
                }`}
              >
                Customer Chat
              </button>
          ...
          >
            {activePane === "chat" ? (
                ...
                <div className="flex gap-3 items-start mb-6">
                  ...
                  <div className="flex flex-col max-w-[85%]">
                    <div className="bg-surface border border-rule/60 rounded-2xl px-4 py-3 text-sm text-ink rounded-tl-none shadow-sm">
                      <p className="font-bold text-xs text-graphite dark:text-graphite mb-1">
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex flex-col max-w-[80%]">
                        ...
                        <span className={`text-[10px] text-graphite mt-1 px-1 ${isSupport ? "text-right" : ""}`}>
              ...
              <div className="space-y-4">
                {notes.length === 0 ? (
                  <div className="py-12 text-center select-none text-graphite">
                    <Lock className="w-10 h-10 mx-auto mb-3 opacity-30" />
                    <p className="text-sm font-semibold">No internal notes added yet</p>
                    <p className="text-xs text-graphite mt-0.5">Notes are private and visible only to staff.</p>
                  </div>
                ) : (
                  notes.map((note) => (
          ...
          {/* Form input controls */}
          <form onSubmit={handleActionSubmit} className="border-t border-rule p-3 bg-surface flex items-center gap-2">
            <input
              type="text"
              value={inputText}
        ...
        <div className="w-full lg:w-80 space-y-4 select-none">
          ...
          {/* Action Box Card */}
          <div className="bg-surface border border-rule rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-1">
              <label htmlFor="ticket-status-select" className="text-[10px] font-bold text-graphite uppercase tracking-wide">Status</label>
              <select
                id="ticket-status-select"
                value={statusVal}
                onChange={(e) => handleStatusChange(e.target.value)}
                className="w-full bg-paper border border-rule rounded-xl text-xs px-3 py-2 text-ink focus:outline-none focus:border-blue-500"
              >
            ...
            {/* Priority Dropdown */}
            <div className="space-y-1">
              <label htmlFor="ticket-priority-select" className="text-[10px] font-bold text-graphite uppercase tracking-wide">Priority</label>
              <select
            ...
            {/* Assign Ticket */}
            <div className="pt-2">
              <p className="text-[10px] font-bold text-graphite uppercase tracking-wide mb-1">Assignment</p>
              {ticket.assignedTo ? (
                ...
                <button
                  onClick={handleSelfAssign}
                  className="w-full text-center border border-dashed border-rule rounded-xl py-2 text-xs font-semibold text-blue-600 dark:text-blue-400 hover:bg-blue-50/20 hover:border-blue-200 transition-colors"
                >
                  Assign to Me
                </button>
              )}
          ...
          <div className="bg-surface border border-rule rounded-2xl p-5 shadow-sm space-y-4">
            ...
            <div className="space-y-3.5 divide-y divide-rule text-xs">
              <div className="pt-3 first:pt-0">
                <p className="text-[10px] font-semibold text-graphite uppercase tracking-wide">Category</p>
                <p className="font-semibold text-ink mt-0.5 capitalize">{ticket.category}</p>
              </div>

              <div className="pt-3">
                <p className="text-[10px] font-semibold text-graphite uppercase tracking-wide">Customer</p>
                <p className="font-semibold text-ink mt-0.5">{ticket.name}</p>
              ...
              <div className="pt-3">
                <p className="text-[10px] font-semibold text-graphite uppercase tracking-wide flex items-center gap-1">
                  <Calendar className="w-3 h-3 text-graphite" />
                  Created
                </p>
                <p className="font-semibold text-graphite dark:text-graphite mt-0.5">
                  {new Date(ticket.createdAt).toLocaleString("en-IN")}
                </p>
              </div>
              ...
              <div className="pt-3">
                <p className="text-[10px] font-semibold text-graphite uppercase tracking-wide flex items-center gap-1">
                  <Layers className="w-3 h-3 text-graphite" />
                  System Details
                </p>
                <p className="text-[10px] text-graphite dark:text-graphite font-mono mt-1 break-words bg-paper p-2 rounded-lg leading-relaxed border border-rule/40">
                  IP: {ticket.ip || "N/A"}<br />
